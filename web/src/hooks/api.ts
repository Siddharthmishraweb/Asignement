...
class APIClient {
  ...
  async fetch<T>(endpoint: string, options?: RequestInit): Promise<T> {
    const normalizedEndpoint =
      this.baseURL.endsWith("/api/v1") && endpoint.startsWith("/api/v1")
...
export const useAlertCounts = () => {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<{ total: number; open: number }>("/alerts/count"),
    refetchInterval: 15000, // refresh every 15s for near-real-time badge
    staleTime: 10000,
  });
};
...
export const useMarkFalsePositive = () => {
  ...
  return useMutation({
    mutationFn: ({ alertId, reason }: { alertId: string; reason?: string }) =>
      apiClient.post("/api/v1/action/mark-false-positive", { alertId, reason }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["alerts"] });
      queryClient.invalidateQueries({ queryKey: ["alerts-count"] });
    },
...
export const useTransactions = (params?: {
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<ApiResponse<Transaction>>("/api/v1/transactions", params),
...
export const useTransaction = (transactionId: number) => {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<Transaction>(`/api/v1/transactions/${transactionId}`),
...
export const useCustomers = (params?: {
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<ApiResponse<Customer>>("/api/v1/customers", params),
...
export const useCustomerInsightsSummary = (
  customerId: string,
  lastDays: number = 90,
) => {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<{
        ...
        generated_at: string;
      }>(`/insights/${customerId}/summary`, { lastDays }),
    enabled: !!customerId,
    staleTime: 5 * 60 * 1000,
  });
};
...
export const useEvaluation = (evaluationId: number) => {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<Evaluation>(`/api/v1/evaluations/${evaluationId}`),
    enabled: !!evaluationId,
  });
};
export const useEvaluationResults = (evaluationId: number) => {
  return useQuery({
    queryKey: ["evaluation-results", evaluationId],
    queryFn: () =>
      apiClient.get<EvaluationResults>(
        `/api/v1/evaluations/${evaluationId}/results`,
      ),
    enabled: !!evaluationId,
    staleTime: 60000, // Results don't change frequently
  });
};
...
export const useEvaluationPerformance = (params?: { days?: number }) => {
  return useQuery({
    queryKey: ["evaluation-performance", params],
    queryFn: () => apiClient.get(`/api/v1/evaluations/performance`, params),
    staleTime: 300000, // 5 minutes - performance data changes slowly
  });
};
