...
import { useAlertCounts, useDashboardMetrics } from "../../hooks/api";
...
export default function Sidebar({ onClose }: SidebarProps) {
  ...
  const { data: counts, isLoading, isError } = useAlertCounts();
  const { data: metrics, isLoading: metricsLoading } = useDashboardMetrics();
  const activeCases = metrics
    ? ((metrics as any).alerts?.by_status?.OPEN || 0) +
      ((metrics as any).alerts?.by_status?.INVESTIGATING || 0)
    : 0;
  ...
  const avgResponseTime =
    (metrics as any)?.performance?.avg_triage_latency_ms || 0;
  const navigation = navigationBase.map((item) => {
    if (item.name === "Alerts Queue") {
      return {
        ...item,
        badge: isLoading ? "…" : isError ? "!" : (counts?.open ?? 0),
      };
    }
    return item;
  });
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
            <div className="bg-gray-50 rounded-lg p-2">
              ...
              <div className="font-semibold text-gray-900">
                {metricsLoading ? "…" : activeCases}
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg p-2">
              <div className="text-gray-500">Avg Response</div>
              <div className="font-semibold text-gray-900">
                {metricsLoading ? "…" : formatResponseTime(avgResponseTime)}
              </div>
            </div>
          </div>
        </div>
