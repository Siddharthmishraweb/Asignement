...
const mockTransactions = Array.from({ length: 50 }, (_, i) => ({
  ...
  currency: "USD",
  transaction_type: ["purchase", "withdrawal", "transfer", "deposit"][
    Math.floor(Math.random() * 4)
  ],
  merchant_name: ["Amazon", "Walmart", "Target", "Starbucks", "Gas Station"][
    Math.floor(Math.random() * 5)
  ],
  merchant_category: ["retail", "food", "gas", "online"][
    Math.floor(Math.random() * 4)
  ],
...
const mockAlerts = Array.from({ length: 20 }, (_, i) => ({
  ...
  type: ["fraud", "compliance", "security", "risk"][
    Math.floor(Math.random() * 4)
  ],
  customer_id: Math.floor(Math.random() * 1000) + 1,
...
app.get("/api/v1/transactions/metrics", (req, res) => {
  ...
  res.json({
    data: {
      ...
      high_risk_count: mockTransactions.filter(
        (t) => t.risk_score && t.risk_score > 80,
      ).length,
      success_rate: 0.95,
      growth_rate: 0.12,
    },
