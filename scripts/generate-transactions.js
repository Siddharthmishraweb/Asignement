...
const merchantCategories = {
  ...
  5812: {
    name: "Restaurants",
    merchants: ["McDonalds", "Starbucks", "Chipotle", "Subway", "Pizza Hut"],
  },
  5542: {
    name: "Gas Stations",
  ...
  5331: {
    name: "Variety Stores",
    merchants: ["Target", "Walmart", "Costco", "Amazon", "Best Buy"],
  },
  4121: {
    name: "Taxi/Rideshare",
    merchants: ["Uber", "Lyft", "Yellow Cab", "QuickCab", "Metro Taxi"],
  },
  5999: {
    name: "Misc Retail",
    merchants: [
...
function generateSuspiciousTransaction(customerId, cardId, accountId, baseTs) {
  ...
  return {
    ...
    currency: "USD",
    ts: baseTs,
    deviceId: factor.velocityGroup
      ? randomElement(deviceIds.slice(0, 5))
      : randomElement(deviceIds),
...
function generateTransactions() {
  ...
  console.log(
    `Writing ${transactions.length} transactions to ${config.outputFile}`,
  );
  const writeStream = fs.createWriteStream(config.outputFile);
  writeStream.write("[\n");
  const writeBatchSize = 10000;
  for (let i = 0; i < transactions.length; i += writeBatchSize) {
    const batch = transactions.slice(i, i + writeBatchSize);
    for (let j = 0; j < batch.length; j++) {
      const txn = batch[j];
      const isLast = i + j === transactions.length - 1;
      writeStream.write("  " + JSON.stringify(txn) + (isLast ? "\n" : ",\n"));
    }
    if (i % 100000 === 0) {
      console.log(`Written ${i + writeBatchSize} transactions...`);
    }
  }
  writeStream.write("]\n");
  writeStream.end();
  ...
  const stats = {
    ...
    customerDistribution: config.customers.reduce((acc, customerId) => {
      acc[customerId] = transactions.filter(
  ...
  console.log(
    `Date range: ${new Date(stats.dateRange.start).toISOString()} to ${new Date(stats.dateRange.end).toISOString()}`,
  );
  console.log(
    `Total amount: $${(stats.totalAmountCents / 100).toLocaleString()}`,
  );
