...
async function runFreezeOtpTest(evaluation) {
  ...
  try {
    const triageResponse = await api.post(
      "/triage",
      {
    ...
    if (triageResult.recommendation !== expectedOutcome.recommendation) {
      errors.push(
        `Expected recommendation ${expectedOutcome.recommendation}, got ${triageResult.recommendation}`,
      );
      passed = false;
    }
    if (triageResult.riskLevel !== expectedOutcome.riskLevel) {
      errors.push(
        `Expected risk level ${expectedOutcome.riskLevel}, got ${triageResult.riskLevel}`,
      );
      passed = false;
    }
    if (expectedOutcome.otpRequired) {
      const freezeResponse = await api.post(
        "/action/freeze-card",
        {
      ...
      if (freezeResponse.data.status !== "FROZEN") {
        errors.push(
          `Expected card to be frozen, got status ${freezeResponse.data.status}`,
        );
        passed = false;
      }
    }
...
async function runDisputeCreationTest(evaluation) {
  logInfo(`Running ${evaluation.name}...`);
  try {
    const disputeResponse = await api.post(
      "/action/open-dispute",
      {
    ...
    if (disputeResponse.data.status !== expectedOutcome.status) {
      errors.push(
        `Expected status ${expectedOutcome.status}, got ${disputeResponse.data.status}`,
      );
      passed = false;
    }
    if (passed) {
...
async function runRateLimitTest(evaluation) {
  ...
  try {
    ...
    if (passed) {
      logSuccess(
        `${evaluation.name} - PASSED (${rateLimitedResponses}/${requestCount} rate limited)`,
      );
      results.passed++;
    } else {
      logError(`${evaluation.name} - FAILED`);
...
async function runEvaluations() {
  ...
  try {
    ...
    logError(
      "Ensure the API server is running and healthy before running evaluations",
    );
    process.exit(1);
  }
