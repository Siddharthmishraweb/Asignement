...
export class KBAgent {
  ...
  async search(query: string, limit: number = 5): Promise<KBResponse> {
    ...
    try {
      ...
      const results = (await this.database.query(searchQuery, [
        searchTerm,
        limit,
      ])) as any[];
  ...
  async getMultipleCitations(
    topics: string[],
  ): Promise<{ [topic: string]: KBSearchResult | null }> {
  ...
  async getAllDocuments(): Promise<any[]> {
    try {
      ...
      this.logger.error("Failed to get all documents", {
        error: error.message,
      });
      return [];
    }
