...
export class InsightsAgent {
  ...
  async generateInsights(
    ...
    try {
      ...
      const totalAmount = transactions.reduce(
        (sum: number, t: any) => sum + t.amount_cents / 100,
        0,
      );
  ...
  private detectAnomalies(transactions: any[], averageAmount: number): any[] {
    ...
    const largeTransactions = transactions.filter(
      (t) => t.amount_cents / 100 > averageAmount * 5,
    );
    ...
    transactions.forEach((t) => {
      merchantFreq.set(
        t.merchant_name,
        (merchantFreq.get(t.merchant_name) || 0) + 1,
      );
    });
    const highFreqMerchants = Array.from(merchantFreq.entries()).filter(
      ([_, count]) => count > 10,
    );
    ...
    const internationalTxns = transactions.filter(
      (t) => t.country && t.country !== "US",
    );
