...
export class FraudAgent {
  ...
  async analyzeRisk(
    ...
    try {
      ...
      `;
      const history = (await this.database.query(historyQuery, [
        customerId,
      ])) as any[];
      ...
      analysis.recommended_action = this.getRecommendedAction(
        analysis.risk_level,
        analysis.score,
      );
  ...
  private checkVelocity(transactions: any[]): {
    score: number;
    reasons: string[];
  } {
    const now = new Date();
    ...
    const recent24h = transactions.filter(
      (t) => new Date(t.transaction_date) >= last24h,
    );
    const recentHour = transactions.filter(
      (t) => new Date(t.transaction_date) >= lastHour,
    );
    const score = Math.min(recent24h.length * 0.5 + recentHour.length * 2, 15);
    const reasons: string[] = [];
    if (recent24h.length > 20) {
      reasons.push(
        `High velocity: ${recent24h.length} transactions in 24 hours`,
      );
    }
    if (recentHour.length > 5) {
      reasons.push(
        `Burst activity: ${recentHour.length} transactions in last hour`,
      );
    }
    return { score, reasons };
  }
  private checkAmountAnomalies(transactions: any[]): {
    score: number;
    reasons: string[];
  } {
    ...
    if (maxAmount > avgAmount * 10) {
      score += 8;
      reasons.push(
        `Large transaction detected: $${maxAmount.toFixed(2)} vs avg $${avgAmount.toFixed(2)}`,
      );
    } else if (maxAmount > avgAmount * 5) {
      score += 4;
      reasons.push(
        `Above-average transaction: $${maxAmount.toFixed(2)} vs avg $${avgAmount.toFixed(2)}`,
      );
    }
    const roundNumbers = amounts.filter(
      (amt) => amt % 100 === 0 && amt >= 1000,
    );
    if (roundNumbers.length > amounts.length * 0.3) {
      score += 3;
      reasons.push(
        `Suspicious round number pattern: ${roundNumbers.length}/${amounts.length} transactions`,
      );
    }
    return { score, reasons };
  }
  private checkMerchantRisk(transactions: any[]): {
    score: number;
    reasons: string[];
  } {
    ...
    transactions.forEach((t) => {
      merchantCategories.set(
        t.merchant_category,
        (merchantCategories.get(t.merchant_category) || 0) + 1,
      );
    ...
    for (const [mcc, count] of merchantCategories.entries()) {
      if (highRiskMCCs.includes(mcc)) {
        score += count * 2;
        reasons.push(
          `High-risk merchant category: ${mcc} (${count} transactions)`,
        );
      }
    }
    ...
    if (topMerchant && topMerchant[1] > transactions.length * 0.4) {
      score += 3;
      reasons.push(
        `High concentration at ${topMerchant[0]}: ${topMerchant[1]} transactions`,
      );
    }
    return { score, reasons };
  }
  private checkGeographicRisk(transactions: any[]): {
    score: number;
    reasons: string[];
  } {
    ...
    const reasons: string[] = [];
    if (countries.size > 1) {
      const internationalCount = Array.from(countries).filter(
        (c) => c !== "US",
      ).length;
  ...
  private checkDevicePatterns(transactions: any[]): {
    score: number;
    reasons: string[];
  } {
  ...
  private async checkChargebackHistory(
    customerId: string,
    ...
    try {
      ...
      `;
      const result = (await this.database.query(chargebackQuery, [
        customerId,
      ])) as any[];
      ...
      this.logger.warn("Failed to check chargeback history", {
        customerId,
        error,
      });
  ...
  private getRecommendedAction(
    riskLevel: string,
    score: number,
  ): "monitor" | "review" | "freeze_card" | "block_transaction" {
    switch (riskLevel) {
      case "critical":
