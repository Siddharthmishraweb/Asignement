...
export class MultiAgentOrchestrator {
  ...
  async startTriage(
    ...
    if (this.database) {
      try {
        ...
        this.logger.error("Failed to persist triage_runs start", {
          runId,
          error: err.message,
        });
      }
    ...
    try {
      for (let i = 0; i < plan.steps.length; i++) {
        ...
        if (totalDuration >= plan.budget_ms) {
          this.logger.warn("Triage budget exceeded", {
            runId,
            step: step.step,
          });
          break;
        }
        this.logger.info("Executing triage step", {
          runId,
          step: step.step,
          agent: step.agent,
        });
        ...
        if (this.database) {
          try {
            ...
            this.logger.error("Failed to persist agent trace", {
              runId,
              step: step.step,
              error: err.message,
            });
        ...
        if (!result.success && step.retries === 0) {
          this.logger.error("Step failed without retries", {
            runId,
            step: step.step,
            error: result.error,
          });
      ...
      if (this.database) {
        try {
          ...
          this.logger.error("Failed to update triage_runs completion", {
            runId,
            error: err.message,
          });
        }
      ...
      this.logger.error("Triage execution failed", {
        runId,
        error: error?.message,
      });
      await this.emitEvent(runId, {
  ...
  private async executeStep(
    ...
    if (this.isCircuitOpen(step.agent)) {
      this.logger.warn("Circuit breaker open for agent", {
        runId,
        agent: step.agent,
      });
    ...
    while (true) {
      try {
        ...
        if (result.success) {
          ...
          this.metrics?.recordAgentLatency(
            step.agent,
            step.step,
            true,
            result.duration_ms,
  ...
  private sanitizeContext(context: TriageContext): TriageContext {
    const unsafePattern =
      /(ignore (?:previous|earlier) instructions|system override|exfiltrate)/i;
    const scrub = (v?: string) =>
      v && unsafePattern.test(v) ? v.replace(unsafePattern, "[redacted]") : v;
    return {
      ...context,
      alertId: scrub(context.alertId) || context.alertId,
      customerId: scrub(context.customerId) || context.customerId,
      transactionId: scrub(context.transactionId) || context.transactionId,
    };
  }
  ...
  private async callAgent(
    context: TriageContext,
    step: TriageStep,
  ): Promise<Omit<AgentResult, "duration_ms">> {
    switch (step.agent) {
      case "insights":
      ...
      case "compliance":
        return this.validateAgentOutput(
          "compliance",
          this.getMockComplianceCheck(),
        );
      default:
  ...
  private validateAgentOutput(
    ...
    try {
      ...
      return {
        success: false,
        data: this.getFallbackData(agent),
        fallback_used: true,
      };
    }
  }
  ...
  private getMockInsights() {
    return {
      ...
      merchants: [
        { name: "Uber", count: 12, amount: 240.5 },
        { name: "Starbucks", count: 8, amount: 64.0 },
      ],
      anomalies: [
      ...
      results: [
        {
          ...
          extract:
            "When multiple transactions occur within a short timeframe...",
        },
      ],
    };
  }
  private getMockComplianceCheck() {
    return {
      ...
      policy_checks: {
        amount_threshold: {
          passed: false,
          details: "Amount exceeds $500 limit",
        },
