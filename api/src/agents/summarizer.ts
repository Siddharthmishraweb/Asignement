...
export class SummarizerAgent {
  ...
  private initializeTemplates(): void {
    this.templates.set("freeze_card_high", {
      customer_message:
        "We've temporarily secured your card due to unusual activity. Your account remains safe and we'll contact you within 24 hours to verify recent transactions.",
      internal_note:
        "Card frozen due to high-risk activity detection. Risk score: {risk_score}. Primary concerns: {fraud_reasons}",
      action_type: "FREEZE_CARD",
      risk_threshold: "high",
    });
    this.templates.set("freeze_card_medium", {
      customer_message:
        "We've temporarily paused your card as a precautionary measure. Please call us at your earliest convenience to verify recent activity.",
      internal_note:
        "Preventive card freeze for medium-risk activity. Risk score: {risk_score}. Factors: {fraud_reasons}",
      action_type: "FREEZE_CARD",
      risk_threshold: "medium",
    });
    this.templates.set("open_dispute", {
      customer_message:
        "We've initiated a dispute for the transaction you reported. You'll receive updates within 5-7 business days. A provisional credit may be applied to your account.",
      internal_note:
        "Dispute opened for transaction {transaction_id}. Amount: {amount}. Reason: {dispute_reason}",
      action_type: "OPEN_DISPUTE",
    });
    this.templates.set("contact_customer", {
      customer_message:
        "We're reaching out regarding recent activity on your account. Please respond at your earliest convenience to help us verify your identity and transactions.",
      internal_note:
        "Customer contact initiated for verification. Method: {contact_method}. Reason: {contact_reason}",
      action_type: "CONTACT_CUSTOMER",
    });
    this.templates.set("false_positive", {
      customer_message:
        "After review, we've determined the recent activity on your account is legitimate. No further action is required.",
      internal_note:
        "Alert marked as false positive. Original risk score: {risk_score}. Review notes: {review_notes}",
      action_type: "FALSE_POSITIVE",
    });
    this.templates.set("review_required", {
      customer_message:
        "We're reviewing recent activity on your account and may contact you for additional verification. Your account remains fully functional.",
      internal_note:
        "Manual review required. Risk level: {risk_level}. Compliance status: {compliance_status}",
  ...
  private generateInternalNote(template: any, context: SummaryContext): string {
    ...
    const fraudReasonsText =
      context.fraud_reasons?.join(", ") || "No specific reasons identified";
    const complianceStatus =
      context.compliance_result?.overall_status || "not_checked";
  ...
  private generateNextSteps(
    ...
    if (context.compliance_result?.required_actions) {
      steps.push(
        ...context.compliance_result.required_actions.map(
          (action: string) => `Complete ${action.replace(/_/g, " ")}`,
        ),
      );
    }
    if (context.compliance_result?.otp_required) {
  ...
  private generateFallbackSummary(
    ...
    return {
      customer_message:
        "We're currently reviewing activity on your account. We'll contact you if any additional verification is needed.",
