...
async function main() {
  ...
  try {
    ...
    triageRows = await db.query(
      "SELECT alert_id, recommended_action, fallback_used, latency_ms FROM triage_runs WHERE started_at > NOW() - INTERVAL '1 day'",
    );
  } catch (e) {
    console.warn(
      "Could not query triage_runs (database not available or migrations not run):",
      (e as Error).message,
    );
  }
  ...
  for (const fx of fixtures) {
    ...
    const actual = run
      ? actionNormalize(run.recommended_action || "")
      : "UNKNOWN";
    if (!metrics.confusion[expected]) metrics.confusion[expected] = {};
    metrics.confusion[expected][actual] =
      (metrics.confusion[expected][actual] || 0) + 1;
  ...
  console.log(
    `OTP match rate: ${((metrics.otpMatches / metrics.total) * 100).toFixed(2)}%`,
  );
  console.log(
    `Fallback rate: ${metrics.total ? ((metrics.fallbackUsed / metrics.total) * 100).toFixed(2) : "0.00"}%`,
  );
