...
export class Database {
  ...
  public async runMigrations(): Promise<void> {
    ...
    try {
      ...
      if (result && result.length > 0 && result[0].exists === true) {
        ...
        await this.ensureEvaluationTables();
        return;
      }
    } catch (error) {
      logger.debug("Migration check failed, proceeding with migrations", {
        error,
      });
    }
  ...
  private async ensureEvaluationTables(): Promise<void> {
    try {
      const evalTableCheck = await this.query(
        "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'evaluations');",
      );
      if (!evalTableCheck[0].exists) {
        logger.info("Creating evaluation tables...");
        const fs = require("fs");
        const path = require("path");
        const evaluationMigrationPath = path.join(
          __dirname,
          "../../migrations/add_evaluations.sql",
        );
        if (fs.existsSync(evaluationMigrationPath)) {
          const sql = fs.readFileSync(evaluationMigrationPath, "utf8");
          await this.query(sql);
          logger.info("Evaluation tables created successfully");
        }
      }
    } catch (error) {
      logger.error("Failed to ensure evaluation tables", { error });
    }
  }
}
export class QueryBuilder {
  ...
  public whereBetween(column: string, start: any, end: any): QueryBuilder {
    ...
    this.conditions.push(
      `${column} BETWEEN $${this.paramCount - 1} AND $${this.paramCount}`,
    );
