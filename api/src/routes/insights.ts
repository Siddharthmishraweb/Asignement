...
export const createInsightsRouter = (config: RouteConfig): Router => {
  ...
  router.get(
    ...
    asyncErrorHandler(async (req: Request, res: Response) => {
      ...
      logger.info("Fetching insights summary", {
        requestId,
        customerId,
        lastDays,
      });
      ...
      try {
        const days = Math.min(
          Math.max(parseInt(lastDays as string, 10) || 90, 1),
          365,
        );
        ...
        logger.error("Insights summary failed", {
          requestId,
          customerId,
          error: error.message,
        });
    ...
    asyncErrorHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const [customerData, interactionData] = await Promise.all([
          database.query("SELECT * FROM customer_data WHERE customer_id = $1", [
            query.customerId,
          ]),
          database.query(
      ...
      try {
        ...
        const analysis = {
          ...
          summary: {
            ...
            periodEnd: endDate,
            totalInteractions: trends[0].reduce(
    ...
    asyncErrorHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const predictions = {
          ...
          nextContactProbability:
            calculateNextContactProbability(customerHistory),
...
function getSentimentTrend(
  interactions: any[],
): Array<{ date: string; sentiment: number }> {
  return interactions
...
function generateRecommendations(customer: any, interactions: any[]): string[] {
  ...
  if (interactions.length > 10) {
    recommendations.push(
      "Consider proactive outreach due to high interaction volume",
    );
  }
  const avgSentiment =
    interactions
      .filter((i) => i.sentiment_score !== null)
      .reduce((sum, i) => sum + parseFloat(i.sentiment_score || 0), 0) /
    interactions.length;
  if (avgSentiment < 0.3) {
    recommendations.push(
      "Priority customer service attention needed due to negative sentiment",
    );
  }
...
function calculateAverageSentiment(sentimentData: any[]): number {
  ...
  return (
    sentimentData.reduce((sum, d) => sum + (d.value || 0), 0) /
...
function calculateAverageResolutionTime(resolutionData: any[]): number {
  ...
  return (
    resolutionData.reduce((sum, d) => sum + (d.value || 0), 0) /
    resolutionData.length
  );
}
function calculateChurnRisk(history: any[]): {
  score: number;
  factors: string[];
} {
  const factors = [];
...
function calculateNextContactProbability(history: any[]): number {
  ...
  const daysSinceLastContact =
    (Date.now() - new Date(history[history.length - 1].created_at).getTime()) /
    (1000 * 60 * 60 * 24);
  return Math.min(daysSinceLastContact / avgInterval, 1.0);
}
function predictSatisfaction(history: any[]): {
  predicted: number;
  confidence: number;
} {
...
function assessEscalationRisk(history: any[]): {
  ...
  const unresolvedCount = recentHistory.filter(
    (h) => h.status !== "resolved",
  ).length;
