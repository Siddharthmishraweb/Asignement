  ...
  router.get("/count", async (_req: Request, res: Response): Promise<void> => {
    try {
      const totalRows = await database.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM alerts WHERE status != 'FALSE_POSITIVE'`,
      );
      const openRows = await database.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM alerts WHERE status = 'OPEN'`,
      );
      res.json({
        total: parseInt(totalRows[0].count, 10),
        open: parseInt(openRows[0].count, 10),
      });
    } catch (error: any) {
      logger.error("Failed to fetch alerts count", { error });
      res.status(500).json({
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      `;
        ...
        logger.error("Failed to assign alert", {
          error,
