...
export const actionRoutes = (config: RouteConfig): Router => {
  ...
  const storeIdempotentResult = async (req: Request, result: any) => {
    ...
    if (idempotencyKey) {
      try {
        ...
        logger.error("Failed to store idempotent result", {
          error,
          idempotencyKey,
        });
      }
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        if (requiresOtp && !otp && !bypassOtp) {
          metrics.incrementCounter("action_blocked_total", {
            policy: "otp_required",
          });
          const result = {
            status: "PENDING_OTP",
        ...
        metrics.incrementCounter("tool_call_total", {
          tool: "freeze_card",
          ok: "true",
        });
        const result = {
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      const requestId = uuidv4();
      try {
        const { txnId, reasonCode, confirm, customerId }: OpenDisputeRequest =
          req.body;
  ...
  router.post(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      const requestId = req.get("X-Request-ID") || uuidv4();
      try {
        const { alertId, reason }: { alertId: string; reason?: string } =
          req.body;
        const auth = (req as any).auth;
        if (!alertId) {
          res.status(400).json({
        ...
        try {
          ...
          logger.warn("Failed to create case for false positive", {
            alertId,
            requestId,
  ...
  async function logAuditEvent(event: {
    ...
    const { data: redactedEvent, masked } =
      RedactorAgent.redactForLogging(event);
    ...
    if (event.caseId) {
      try {
        ...
        logger.error("Failed to insert case_event", {
          error,
          requestId: event.requestId,
          action: event.action,
        });
