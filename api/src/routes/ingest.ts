...
export const createIngestRouter = (config: RouteConfig): Router => {
  ...
  router.post(
    ...
    asyncErrorHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        metrics.incrementCounter("ingest_data_total", {
          type: "customer_data",
        });
        res.json({
          success: true,
        ...
        metrics.incrementCounter("ingest_errors_total", {
          type: "customer_data",
        });
        res.status(500).json({
      ...
      try {
        if (idempotencyKey) {
          ...
          if (cachedResult) {
            ...
            logger.info("Returning cached ingest result", {
              requestId,
              idempotencyKey,
            });
        ...
        metrics.incrementCounter("ingest_transactions_total", {
          status: "completed",
        });
  ...
  router.post(
    ...
    asyncErrorHandler(async (req: Request, res: Response) => {
      ...
      logger.info("Processing batch ingest", {
        requestId,
