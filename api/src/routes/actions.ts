    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (!transactionId || !reasonCode) {
          res
            .status(400)
            .json({ error: "Transaction ID and reason code are required" });
          return;
        }
        const caseQuery = `
        INSERT INTO cases (
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (!customerId || !method || !message) {
          res.status(400).json({
            error: "Customer ID, contact method, and message are required",
          });
          return;
        }
        const logQuery = `
      ...
      `;
        ...
        logger.info("Customer contacted", {
          customerId,
          method,
          communicationId,
        });
        ...
        logger.error("Failed to contact customer", {
          error,
