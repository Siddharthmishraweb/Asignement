...
process.env.DATABASE_URL =
  ...
  "postgresql://test:test@localhost:5432/sentinel_test";
...
async function teardownTestDatabase(): Promise<void> {
  try {
    await global.testDatabase.query(
      "TRUNCATE customer_data, interactions, knowledge_articles RESTART IDENTITY CASCADE",
    );
  } catch (error) {
...
export const TestUtils = {
  createTestCustomer: async (
    customerId: string = "test-customer-001",
    data: any = {},
  ) => {
    const customerData = {
      customerId,
